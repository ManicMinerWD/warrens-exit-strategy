...
const BALI_STRATEGY = {
  ...
  steps: [
    ...
    {
      title: "Alternative deployment: leave IDR in-country and invest in a Local Market Mandiri Bank product paying ~11% dividend (Option C — in-country yield, no FX, no SG tax)",
      detail: "Instead of converting IDR → SGD and deploying to IBKR (Option B), the after-10%-tax IDR is left in the Mandiri corporate account and deployed into a local-market Mandiri Bank instrument (e.g. a term deposit / mandatory deposit / locally-issued money-market product) that pays ~11% dividend/yield in IDR. This avoids the 1–2% FX spread, avoids the 22% SG capital gains tax entirely (no SG event), and avoids the 20% outbound dividend withholding (no dividend, no offshore transfer). The trade-off: the ~11% yield is IDR-denominated (inflation risk against SGD), may be taxed separately at the Indonesian corporate rate or a final rate on top of the 10% rental tax (confirm — is the Mandiri yield also 10% final, or is it grossed up and taxed at the corporate rate?), and has a lower growth ceiling than a 15-year global AI/robotics share portfolio. Option C is the in-country yield play — suitable if the priority is IDR yield without FX/tax leakage rather than long-horizon capital growth. The 10B IDR capital investment requirement can still be met with in-country deployment (the villa purchases are the capital investment — Option C does not replace the villa purchases; it is the deployment vehicle for the rental income, not the capital investment vehicle).",
      caveat: "Confirm the exact Mandiri Bank instrument, the 11% yield (gross or net, fixed or variable, payout frequency, minimum balance, lock-up, early-redemption terms), and the Indonesian tax treatment of the yield (final 10%? corporate rate? — this is separate from the 10% rental tax and must be confirmed independently). Option C does not change the villa-purchase + 10B IDR capital investment loop; the villa purchases are still the capital investment toward the PT PMA's 10B IDR requirement. The 11% is a yield on deployed balances, not a guarantee — rates move with Bank Indonesia policy; confirm current rate environment with the Mandiri relationship manager."
    },
    {
  ...
  taxComparison: {
    ...
    optionB: {
      label: "Option B — convert to SGD and deploy as capital into IBKR (preferred for capital growth)",
    ...
    optionC: {
      label: "Option C — leave IDR in-country and invest in a Local Market Mandiri Bank product paying ~11% dividend",
      step1: "No dividend, no FX: the after-10%-tax IDR stays in the Mandiri corporate account (or a designated investment sub-account) and is deployed into a local-market Mandiri Bank instrument (e.g. a term deposit / mandatory deposit / locally-issued money-market instrument) that pays ~11% dividend/yield in IDR. No IDR→SGD conversion step, so the 1–2% FX spread is avoided entirely.",
      step2: "Net IDR after the 10% rental tax: ~90% of gross rental stays in IDR; the ~11% yield is earned in IDR on the deployed balance (not on the tax). Confirm the exact instrument, yield (is the 11% a gross or net rate, fixed or variable, frequency of payout), minimum balance, lock-up, and early-redemption terms with the Mandiri relationship manager.",
      step3: "Indonesian tax on the Mandiri yield: the 11% dividend/yield is Indonesian-sourced income to the PT PMA and is itself subject to Indonesian tax — confirm the tax treatment (is it final 10% withholding like the rental, or is it grossed-up and taxed at the corporate rate? is there a final tax on bank deposit interest for PMA entities?). The rental income was taxed at 10% final; the Mandini yield may be taxed differently — do not assume it is also 10% final without confirmation.",
      step4: "No 22% SG capital gains tax: because the capital never leaves Indonesia and is never converted to SGD, there is no SG tax event. This is the main advantage over Option B — but the trade-off is IDR denomination (inflation risk against SGD), lower growth ceiling (an 11% IDR instrument has a different growth profile than a 15-year global AI/robotics share portfolio), and no capital-gains compounding in a separate jurisdiction.",
      totalLeakage: "10% rental tax only (the 11% yield is earned after the 10% rental tax on the balance; the yield itself may be separately taxed — confirm). No 20% dividend withholding, no FX drag, no 22% SG gains tax — but the yield is in IDR and may be taxed at the Indonesian corporate rate or a final rate depending on the instrument. Net return is 11% IDR minus any additional Indonesian tax on the yield, versus the Option B path of (10% rental tax) → SGD → 22%-taxed SG gains on a globally diversified growth portfolio."
    },
    summary: "Three deployment paths from the PT PMA's after-10%-tax IDR: Option A (pay dividend to SG shareholder) leaks ~30%+ (10% + 20% DTA + FX) before deployment. Option B (convert to SGD, deploy to IBKR as capital) limits leakage to 10% + FX, with 22% only on SG gains when realised — the preferred path for capital growth over 15 years. Option C (leave IDR in-country, invest in a Local Market Mandiri Bank product paying ~11% dividend) avoids FX and the 20% DTA entirely and pays an IDR-denominated yield, but the yield is in IDR (inflation risk vs SGD), may be taxed at the Indonesian corporate rate or a final rate on top of the 10% rental tax, and has a lower growth ceiling than a global share portfolio. Option C is the in-country yield play; Option B is the capital-growth play; the choice depends on whether the priority is IDR yield without FX/tax leakage (Option C) or long-horizon global capital growth with 22%-only-on-gains (Option B)."
  },
  risks: [
    ...
    { num: "8", title: "Alternative deployment (Option C — Local Market Mandiri Bank ~11% dividend, in-country)", body: "The after-10%-tax IDR can instead of being converted to SGD and deployed to IBKR be left in-country and deployed into a local-market Mandiri Bank instrument (term deposit / mandatory deposit / money-market product) that pays ~11% dividend/yield in IDR. This avoids FX (no IDR→SGD conversion), avoids the 20% outbound dividend withholding, and avoids the 22% SG capital gains tax entirely — but the yield is IDR-denominated (inflation risk vs SGD), may be taxed separately at the Indonesian corporate rate or a final rate on top of the 10% rental tax, and has a lower growth ceiling than a 15-year global share portfolio. The 11% is a yield on deployed balances, not a guarantee — Bank Indonesia rates move. Confirm: exact Mandiri instrument, yield (gross/net, fixed/variable, frequency, minimum balance, lock-up, redemption terms), and the Indonesian tax treatment of the yield (final 10%? corporate rate? — separate from the 10% rental tax). Option C does not replace the villa-purchase + 10B IDR capital investment loop — it is the deployment vehicle for the rental income, not the capital investment vehicle." }
  ],
  deploymentTargets: [
    ...
    { layer: "Mandiri corporate account", what: "IDR bank account in PT PMA name — collects rental, pays expenses; can be the deployment vehicle itself (Option C: local-market Mandiri Bank product paying ~11% dividend) or the FX bridge to SG IBKR (Option B)", why: "The FX + repatriation bridge (Option B) OR the in-country yield vehicle (Option C). 10% rental tax withheld here; net IDR either converted to SGD for IBKR or left in IDR and deployed into a local Mandiri Bank instrument.", status: "Opening pending — after PT PMA ready" },
    { layer: "Local Market Mandiri Bank product (Option C — in-country yield alternative)", what: "A Mandiri Bank local-market instrument (e.g. term deposit / mandatory deposit / locally-issued money-market product) that pays ~11% dividend/yield in IDR — the capital stays in-country, no FX, no SG tax event", why: "Alternative to IBKR: keeps the after-10%-tax IDR in-country, earns ~11% in IDR, avoids the 20% outbound dividend withholding entirely, avoids FX drag, and avoids the 22% SG capital gains tax — but the yield is IDR-denominated (inflation risk vs SGD), may be taxed at the Indonesian corporate rate or a final rate on top of the 10% rental tax, and has a lower growth ceiling than a 15-year global share portfolio. The deployment target is the instrument itself, not shares.", status: "Confirm with Mandiri relationship manager: exact instrument, 11% yield (gross/net, fixed/variable, payout frequency), minimum balance, lock-up, early-redemption terms, and Indonesian tax treatment of the yield (final 10%? corporate rate? — separate from the 10% rental tax)" },
    { layer: "Singapore IBKR account", what: "SGD brokerage account for AI/robotics/sovereign-infrastructure shares — capital gains taxed at 22% on realisation (corporate holder; individual may be exempt)", why: "The deployment destination (Option B). No Indonesian tax on gains; no dividend withholding tax. The after-10%-tax IDR becomes SGD capital deployed over 15 years.", status: "Onboarding pending — after PT PMA + bank ready" },
    { layer: "AI / robotics / sovereign-infrastructure shares", what: "Long-horizon (15yr) equity portfolio — the investment target for the SGD capital. Not detailed on this page.", why: "The capital growth layer (Option B). See separate deployment workstream. This page documents the plumbing only.", status: "Separate workstream" }
  ]
};
